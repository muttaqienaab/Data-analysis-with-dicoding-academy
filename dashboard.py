...
def create_daily_orders_(df):
    daily_orders_df = df.resameple(rule='D',on='order_date').agg({
        "order_id": "nunique",
        "total_price": "sum"
    })
...
def create_sum_order_items_df(df):
    sum_order_items_df = df.groupby("products_name").quantity_x.sum().sort_values(ascending=False).reset_index()
